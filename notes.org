...
** Syntax
   ...
   |  28–BF | refs                  |                        |
   |        |                       |                        |
   |  C0–DF | uint[value]           | value = (marker && 1F) |
   |  E0–FF | array[size] {content} | size = (marker && 1F)  |
   |        |                       |                        |
...
* Bootstrapping a hashing vocabulary
  ...
  - how do you get to the point where you know ~0x28 0x19~ is ~hash:sha3~?
    - you get the list (hopefully with only one element) of
      ...
      64-bits word ~{hashid}~
    - for each of them, you check if ~0x19~ is a name associated with
      a hashing algorithm
      - if yes, you check if that hash matches the definition

* Minimal BULK
  : ( version 1 1 ) ( ns 40 ( 40:sha3 w64 8B ) ) ( ns 41 ( 40:sha3 w64 8B ) ) 41:ref # w? {size} {content}
  : |<---- 8 ---->| |<---------- 19 ---------->| |<---------- 19 ---------->| |<-- 4 -->|  2/4/8
