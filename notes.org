...
** BULK core namespace
   ...
   - 0x09 :: =bulk=
     - =( bulk Bytes )=
     - =( bulk {exprs} )=
       - the semantics is that this has the value of the last expression
   - 0x0A :: ( blob Bytes )
   - 
   - 0x0B :: =( concat {a1} {a2} )=
   - 0x0C :: =indexed-bulk=
     - =( indexed-bulk Nat Nat )=
     - =( indexed-bulk Ref Nat )=
   - 0x0D :: =indexed-array=
     - =( indexed-array Nat Nat Nat )=
     - =( indexed-array Ref Nat Nat )=
   - 
   - 0x0E :: =true=
   - 0x0F :: =false=
   - 
...
** BULK utils
   - decimal2 :: =( subst ( decimal-fixed 2 ) )=
   - prefix-with :: =( subst ( bulk ( define prefix ( arg 0 ) ) ( prefix ( rest 1 ) ) ) )=
   - property-list :: =( subst ( bulk ( define prefix nil ) ( define prefix ( nil true ) ) ( prefix
     ( rest 0 ) ) ) )=
...
** BARK: BULK Archive
   ...
   - 0x01 :: ( described {metadata}:Expr {content}:Expr )
   ...
   -
   - 0x30 :: ( compressed {method}:MeTOD Bytes )
   - 0x31 :: deflate
   - 0x32 :: deflate64
   - 0x33 :: lzma
   - 0x34 :: lzma2
   ...
   - 0x3F :: ( encrypted {method}:Expr Bytes )
   - tar semantics
     - metadata
	 ...
	 - {id} can be anything, incl. string and Nat
	   - multiple entries (e.g. "pierre"/1000: =( user ( string "pierre" ) ) ( user (
...
** BOON
   ...
   BOON adds names to be able to encode some common types of values found in JSON and formats with
   a semantics derived from it, like MessagePack or CBOR. An expression that is one of BOON's cases
   has type =Boon=.
...
** TODO Bootstrapping a hashing vocabulary
   ...
   - the application reads ~( ns 20 ( 0x14-2E #[8] {hashID}:8B ) )~
     - it needs to get to the point where it knows that ~0x14-2E~ is ~hash:shake128~:
       - it looks up in its configuration if there is a namespace identified by a form whose first
	 element is a reference in that same namespace with name ~0x2E~
	 - if there is such an already known namespace, the name ~0x2E~ must be associated with an
           algorithm to match an input identifier to its configuration data
	 - if the algorithm accepts ~{hashid}~, then bootstrapping is done and the namespace is
           available in the rest of the stream
   - the application reads ~( import 20 4 ( 0x15-2E #[8] {hashID}:8B ) )~
     - it needs to get to the point where it knows that ~0x15-2E~ is ~hash:shake128~:
       - it looks up in its configuration if there is a package matching the following conditions:
	 - the package has at least 4 namespaces
	 - the package is identified by a form whose first element is a reference in its second
           namespace, whose name ~0x2E~ must be associated with an algorithm to match an input
           identifier to its configuration data
	 - if the algorithm accepts ~{hashid}~, then bootstrapping is done and the namespace is
           available in the rest of the stream
   - if bootstrapping fails, the application SHOULD consider that it cannot resolve namespaces
     identified by forms using any of the boostrapped namespaces
     - BULK makes it possible to discover new namespaces on-demand but it would be too much of a
       risk to use discovery for primitives that are the foundation of the security model
