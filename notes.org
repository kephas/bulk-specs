...
** BULK core namespace
   ...
   - 0x01 :: true
   - 0x02 :: false
   - 0x03 :: ( ns {marker}:Ref {id}:UniqueID )
   - 0x04 :: ( package {id}:UniqueID {namespaces} )
   - 0x05 :: ( import {base}:Int {count}:Int {uuid} )
...
* Bootstrapping a hashing vocabulary
  - the problem is that this vocabulary provides hashes before any way of expressing a hash is
    possible, so its own hash is expressed with a name inside the vocabulary
  - you read ~( ns 0x1800 ( 0x182E #[8] {hashID}:8B ) )~
  - how do you get to the point where you know ~0x182E~ is ~hash:sha-3~?
    - you get the list (hopefully with only one element) of vocabularies identified by a form whose
      sole element is a 64-bits word ~{hashid}~
