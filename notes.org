...
** BULK core namespace
   ...
   - 0x0A :: ( concat {a1} {a2} )
   - 
   - 0x0B :: ( indexable Nat Bytes )
   - 0x0C :: ( indexed-bulk Nat Nat )
   - 0x0D :: ( indexed-array Nat Nat Nat )
   - 
   - 0x0E :: =true=
   - 0x0F :: =false=
   - 
   ...
   - 0x12 :: ( rest Nat )
   - 
   - 0x13 :: =( unsigned-int Bytes ):Nat=
   - 0x14 :: =( signed-int Bytes ):Int=
   - 0x15 :: =( fraction Int Int )=
   - 0x16 :: =( binary-float Bytes )=
   - 0x17 :: =( decimal-float Bytes )=
   - 0x18 :: =( binary-fixed Nat Bytes )=
   - 0x19 :: =( decimal-fixed Nat Bytes )=
   - 
   - 0x1A :: =prefix=
     - =( prefix {bytecode} )=
   - 0x1B :: =postfix=
   ...
   - 
   - 0x1D :: =( iana-charset Nat ):Encoding=
