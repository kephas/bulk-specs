* BULK
  ...
  - expressions: atoms and forms
	...
	- the namespace identifier is an unsigned integer represented
          either a byte between 0x10 and 0x254 (inclusive) or a
          sequence of one or several 0x255 bytes followed by a byte
          between 0x0 and 0x254;
	  - NSID 0x10 to 0x1F are reserved to be used as standard
            namespaces defined by BULK specifications
	- the reference identifier is an unsigned integer represented
          by a single byte
    - forms are series of expressions between 0x1 and 0x2 bytes
...
** stdlib
   ...
   0xA−0xF atoms reserved for future major versions of BULK.
