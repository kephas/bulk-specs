...
** BULK core namespace
   ...
   - 0x04 :: ( package {id}:Expr {data}:Expr {namespaces} )
   ...
   - 0x17 :: ( indexable Nat Bytes )
   - 0x18 :: ( indexed-bulk Nat Nat )
   - 0x19 :: ( indexed-array Nat Nat Nat )
   - 
...
* Minimal BULK
  : ( version 1 0 ) ( import 20 2 ( hash:shake128 #[8] 8B ) ) some:type #[0–63] {content}
  : |<---- 6 ---->| |<----------------- 19 ---------------->| |<----- 3 ----->|

  : ( version 1 0 ) ( import 20 2 ( hash:shake128 #[8] 8B ) ) some:type # {size} {content}
  : |<---- 6 ---->| |<----------------- 19 ---------------->| |<-- 3 -->| 2/3/5/9
...
** Forward compatibility
   ...
   #+BEGIN_SRC bulk
   ( verifiable-ns 0x1400 ( bark-alpha:sha-3 {bark-alpha-hash} ) nil "bark-alpha"
