...
** TODO Bootstrapping a hashing vocabulary
   ...
   - the application reads ~( ns 18 ( 0x18-2E #[8] {hashID}:8B ) )~
     - it needs to get to the point where it knows that ~0x18-2E~ is ~hash:shake128~:
   ...
   - the application reads ~( import 18 4 ( 0x19-2E #[8] {hashID}:8B ) )~
     - it needs to get to the point where it knows that ~0x19-2E~ is ~hash:shake128~:
	 ...
	 - if the algorithm accepts ~{hashid}~, then bootstrapping is done and the namespace is
           available in the rest of the stream
       - vocabulary identified by a form whose first
